...
BERT_ROOT_PAD = torch.zeros(1, 1024)


def build_data_sets():
    """ 创建 sentence 对象数据集，划分训练数据集 """
    sents_training = build_specific_data_set(RAW_TRAIN_FILES)
    sents_test = build_specific_data_set(RAW_TEST_FILES)
    # sents_dev = random.sample(sents_training, DEV_SIZE)
...
def build_voc(data_set):
    """ word2ids, pos2ids, syn2ids save all these dictionaries.
    """
    # glove words
    words_set = set()
...
def gen_specific_instances(data_set, word2ids, pos2ids, syn2ids):
    ...
    bc = BertClient() if USE_BERT else None
    ...
    for sentence in data_set:
        ...
        if USE_BERT:
            result = bc.encode([sent_token_list], is_tokenized=True)
            # result = bc.encode([" ".join(sent_token_list)], is_tokenized=True)
            for idx in range(len(sent_token_list)):
                tmp_tok = torch.Tensor(result[0][idx + 1]).unsqueeze(0).float()
                tmp_sent_tokens_bert = tmp_tok if tmp_sent_tokens_bert is None else \
                    torch.cat((tmp_sent_tokens_bert, tmp_tok), 0)
            tmp_sent_tokens_bert = torch.cat((BERT_ROOT_PAD, tmp_sent_tokens_bert), 0)
        ...
        data_set_.append((sent_words_ids, sent_poses_ids, graph_ids, sent_tags_ids,
                          tmp_sent_tokens_emb, tmp_sent_tokens_bert))
    p_2.finish()
    return data_set_
