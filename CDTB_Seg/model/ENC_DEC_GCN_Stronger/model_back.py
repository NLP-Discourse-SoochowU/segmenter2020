...
from model.ENC_DEC_GCN.sim_attn import Sim_Attn
...
class Segment_Model(nn.Module):
    ...
    def enc_decode_(self, rnn_inputs, gcn_hidden, decode_mask, decode_indices):
        ...
        bi_affine_attn = self.bi_affine(e_out, d_out).squeeze(-1)
        # input(bi_affine_attn)
        decode_mask = decode_mask.unsqueeze(0).float()
        mask_pad_ = (1 - decode_mask) * SMOO_VAL
        masked_bi_affine_attn = bi_affine_attn.mul(decode_mask) + mask_pad_
        # scoring
        boundary_predict = masked_bi_affine_attn.log_softmax(dim=2)
        # boundary_predict = boundary_predict.mul(decode_mask)
